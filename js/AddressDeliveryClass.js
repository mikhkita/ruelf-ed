        ...
        defineClass(AddressDelivery, {
            ...
            balloonOpen: function (coords) {
                if(!this._map.balloon.isOpen()){
                    ...
                    var price = (this._priceDlivery == 0) ? "бесплатно." : this._priceDlivery + "р.",
                        message = '<span style="font-weight: bold; font-style: italic">'+
                            ( this._priceDlivery > -1 ? "Стоимость доставки: %s" : "Доставка не осуществляется.")
                            +'</span>';


                    $(".field_coordinate_delivery").val(coords);
                    this._balloon = this._map.balloon.open(coords, {
                        contentBody:  this._addressPointBalloonContent + message.replace('%s', price)
                    });
                    this.addressViewDo();
                }
            },
